...
<#
.SYNOPSIS
    Unit Test & Benchmark Runner; builds and runs the test suite and optional scaling benchmark.
...
.EXAMPLE
    ...
    .\test.ps1 -Benchmark -MaxN 8192            # pass -MaxN to benchmark
#>
...
if ($RunBench) {
    Write-Host ""
    Write-Host "Running scaling benchmark..."
    ./build/benchmark.exe --trials $Trials --max-n $MaxN
